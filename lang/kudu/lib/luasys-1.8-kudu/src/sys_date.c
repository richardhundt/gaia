...
/*
 * Arguments: date string, format string, [date table]
 * Returns: date table
 */
static int
sys_parsedate (lua_State *L)
{
    time_t t;
    struct tm ts;
    const char *d = luaL_checkstring(L, 1);
    const char *f = luaL_checkstring(L, 2);
    bzero(&ts, sizeof(struct tm));
    if (!strptime(d, f, &ts)) {
        return sys_seterror(L, 0);
    }
    if (lua_istable(L, -1)) {
        date_setfield(L, "sec", ts.tm_sec);
        date_setfield(L, "min", ts.tm_min);
        date_setfield(L, "hour", ts.tm_hour);
        date_setfield(L, "day", ts.tm_mday);
        date_setfield(L, "month", ts.tm_mon + 1);
        date_setfield(L, "year", ts.tm_year + 1900);
        date_setfield(L, "wday", ts.tm_wday);
        date_setfield(L, "yday", ts.tm_yday + 1);
        date_setfield(L, "isdst", ts.tm_isdst ? ts.tm_isdst : -1);
    }
    else {
